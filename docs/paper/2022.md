...
CREATE TABLE News (
    ...
    NewsText VARCHAR2(4000) NOT NULL,
    EmployeePositionType CHAR(15) NOT NULL,
    DisplayUntil DATE NOT NULL,
...
- **关系模式（Oracle PL/SQL 表结构）**：
  ...
  CREATE TABLE Book (
      ...
      ccl_number VARCHAR2(20)
  );
  
  -- 书籍副本表
  CREATE TABLE BookCopy (
      barcode VARCHAR2(30) PRIMARY KEY,
      isbn VARCHAR2(20) NOT NULL,
      status VARCHAR2(10) DEFAULT 'AVAILABLE',
      CONSTRAINT FK_BookCopy_Book FOREIGN KEY (isbn) REFERENCES Book(isbn)
  );
  ...
  CREATE TABLE Borrow (
      ...
      return_date DATE,
      CONSTRAINT PK_Borrow PRIMARY KEY (sno, barcode, borrow_date),
      CONSTRAINT FK_Borrow_Student FOREIGN KEY (sno) REFERENCES Student(sno),
      CONSTRAINT FK_Borrow_BookCopy FOREIGN KEY (barcode) REFERENCES BookCopy(barcode)
  );
  ...
  CREATE TABLE Reserve (
      ...
      CONSTRAINT PK_Reserve PRIMARY KEY (sno, isbn),
      CONSTRAINT FK_Reserve_Student FOREIGN KEY (sno) REFERENCES Student(sno),
      CONSTRAINT FK_Reserve_Book FOREIGN KEY (isbn) REFERENCES Book(isbn)
  );
  ```
- **范式判断**：
...
**SQL查询**：
```sql
SELECT ccl_number, COUNT(DISTINCT isbn) AS book_count
FROM Book
GROUP BY ccl_number
ORDER BY ccl_number;
```
...
**事实4的存储过程**：
```sql
CREATE OR REPLACE PROCEDURE NotifyReservedStudents (
    returned_barcode IN VARCHAR2
)
IS
    v_email VARCHAR2(255);
    v_subject VARCHAR2(255) := 'Book Returned';
    v_text VARCHAR2(255) := 'The book you reserved is now available.';
    v_returned_isbn VARCHAR2(20);
    v_count NUMBER := 0;
    ...
    CURSOR student_cursor IS
        ...
        WHERE r.isbn = v_returned_isbn
        ORDER BY r.reserve_date;
BEGIN
    -- 获取归还书籍的ISBN
    SELECT isbn INTO v_returned_isbn
    FROM BookCopy
    WHERE barcode = returned_barcode;
    ...
    FOR student_rec IN student_cursor LOOP
        ...
        -- 调用邮件发送函数
        mailto(student_rec.email, v_subject, v_text);
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        DBMS_OUTPUT.PUT_LINE('未找到对应的书籍或预留信息');
    WHEN OTHERS THEN
        DBMS_OUTPUT.PUT_LINE('发生错误: ' || SQLERRM);
END NotifyReservedStudents;
/
```
...
EXCEPTION
    WHEN NO_DATA_FOUND THEN
        NULL; -- 如果没有找到对应的预留信息，不做处理
    WHEN OTHERS THEN
        RAISE_APPLICATION_ERROR(-20001, '删除预留信息时发生错误: ' || SQLERRM);
END trg_delete_reservation;
/
```
:::  
...
BEGIN
    ...
    FOR student_rec IN student_cursor LOOP
        -- 计算该学生读过的书籍数
        SELECT COUNT(DISTINCT bno) INTO v_read_books
        FROM Read
        WHERE sno = student_rec.sno;
        
        -- 如果读过所有书籍，输出学生信息
        IF v_read_books = v_total_books THEN
            DBMS_OUTPUT.PUT_LINE('学号: ' || student_rec.sno || 
                               ', 姓名: ' || student_rec.sname);
        END IF;
    END LOOP;
END;
/
...
```sql
-- Oracle PL/SQL 创建索引语法
CREATE INDEX idx_score_score_sid ON Score (Score, S_ID);
CREATE INDEX idx_student_id ON Student (ID);

-- 也可以考虑创建覆盖索引，包含查询所需的所有列
CREATE INDEX idx_score_covering ON Score (Score, S_ID) 
INCLUDE (S_ID) WHERE Score > 80;
```
